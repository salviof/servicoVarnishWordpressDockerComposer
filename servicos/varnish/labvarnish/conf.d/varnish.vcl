...
sub identifica_agente {
 
  # Default to classification as a PC
  set req.http.X-Device = "pc";
  
 
  if (req.http.User-Agent ~ "(?i)Mobile" || req.http.User-Agent ~ "(?i)Android" || req.http.User-Agent ~ "(?i)Silk" || req.http.User-Agent ~ "(?i)Kindle" || req.http.User-Agent ~ "(?i)BlackBerry" || req.http.User-Agent ~ "(?i)Opera Mini" || req.http.User-Agent ~ "(?i)Opera Mobi" ) 		{
...
call identifica_agente;
//  std.log(req.http);


  if (req.http.host ~ "imagens") {
    set req.backend_hint = backendsRegistrados.backend(req.http.host);
    set req.http.host = req.http.enderecobucket;
    return(hash);
  }
